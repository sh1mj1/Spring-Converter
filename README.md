...
이번에는 일반적인 웹 애플리케이션 환경을 생각해봅시다.. 불린 타입을 숫자로 바꾸는 것 같은 범용 기능보다는 개발자 입장에서는 문자를 다른 타입으로 변환하거나, 다른 타입을 문자로 변환하는 상황이 대부분이다.
...
이렇게 객체를 특정한 포맷에 맞추어 문자로 출력하거나 또는 그 반대의 역할을 하는 것에 특화된 기능이 바로 포맷터(`Formatter`) 입니다. 포맷터는 컨버터의 특별한 버전으로 이해하면 됩니다.
...
`Formatter` 는 문자에 특화 (객체 → 문자, 문자 → 객체) + 현지화(Locale)
        ...
        즉, `Converter` 의 특별한 버전

### 포맷터 - Formatter 만들기

포맷터(`Formatter`)는 객체를 문자로 변경하고, 또 문자를 객체로 변경하는 두 가지 기능을 모두 수행합니다.

`String print(T object, Locale locale)` : 객체를 문자로 변경합니다.

`T parse(String text, Locale locale)` : 문자를 객체로 변경합니다.

`Formatter` 가 상속하는 인터페이스 `Printer` , `Parser`

![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/5382f7bb-a27d-4e45-a1b0-0b0bb1e8143c/Untitled.png)

![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/4cab2fa6-fb36-4d2d-bb73-c9a1e0ea1951/Untitled.png)

![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/fe741df0-f1f9-4ac2-aac5-2b277ad8465a/Untitled.png)

숫자 `1000` 을 문자 `“1,000”` 으로, 즉, 1000 단위로 쉼표가 들어가는 포맷 등을 만들 수 있습니다.. 그리고 그 반대로 처리해주는 포맷터도 만들어봅시다. 아래처럼 만들 수 있습니다.
...
`“1,000”` 처럼 숫자 중간의 쉼표를 적용하려면 자바가 기본으로 제공하는 `NumberFormat` 객체를 사용하면 됩니다. 이 객체는 `Locale` 정보를 활용해서 나라별로 다른 숫자 포맷을 만들어 줍니다.

`parse()` 을 사용해서 문자를 숫자로 변환합니다. (`Number` 타입은 `Integer`, `Long` 같은 숫자 타입의 부모 클래스임)

`print()` 을 사용해서 객체를 문자로 변환합니다.
...
`parse()` 의 결과가 `Long` 이기 때문에 `isEqualTo(1000L)` 을 통해 비교할 때 마지막에 `‘L’` 을 넣어주어야 합니다.
...
> 참고 - 스프링은 용도에 따라 다양한 방식의 포맷터를 제공합니다.
Formatter :  포맷터
